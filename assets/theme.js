...
class Cart {
  ...
  updateQuantity(event, lineItem) {
    ...
    const id = key.substr(0, key.indexOf(':'));
    ...
    return updateItem(key, { quantity: newQuantity })
      ...
      .then((state) => {
        ...
        const totalQuantity = state.items.reduce((total, currentItem) => {
          return currentItem.id === Number(id)
...
class Form {
  constructor() {
    const forms = document.querySelectorAll(selectors$2.form);
  ...
  _focusFormStatus(form) {
    const formStatus = form.querySelector(selectors$2.formStatus);
  ...
  _handleFormInputLabels(form) {
    const inputWrappers = form.querySelectorAll(selectors$2.formInputWrapper);

    if (!inputWrappers) return;

    inputWrappers.forEach((inputWrapper) => {
      inputWrapper.addEventListener('focusin', () => {
        inputWrapper.classList.add(classes$2.floatingLabel);
      });
      ...
      inputWrapper.addEventListener('focusout', (event) => {
        ...
        inputWrapper.classList.remove(classes$2.floatingLabel);
      });
    });
  }
}
...
class CartTemplate {
  ...
  init() {
    if (!cookiesEnabled()) {
      this.elements.cart.classList.add(classes$3.cookiesDisabled);
    }
  }
}
...
class Scroller {
  ...
  init() {
    ...
    this.menu = this.wrapper.querySelector(selectors$3.menu);
    this.arrows = this.container.querySelectorAll(selectors$3.arrow);
  ...
  _setOverflowClass(overflowValue) {
    ...
    this.wrapper.classList.remove(`${classes$4.wrapper}--${this.overflowValue}`);

    window.requestAnimationFrame(() => {
      this.wrapper.classList.add(`${classes$4.wrapper}--${overflowValue}`);
  ...
  _setMenuTranslateX(distance) {
    ...
    this.menu.classList.remove(classes$4.noTransition);
  ...
  _setWrapperScrollLeft() {
    ...
    this.menu.classList.add(classes$4.noTransition);
...
class Disclosure {
  constructor(container) {
    ...
    this.disclosure = container.querySelector(selectors$5.disclosure);
    this.disclosureList = container.querySelector(selectors$5.disclosureList);
    this.disclosureToggle = container.querySelector(selectors$5.disclosureToggle);
    this.disclosureInput = container.querySelector(selectors$5.disclosureInput);
    this.disclosureOptions = container.querySelectorAll(
      selectors$5.disclosureOptions
    );
  ...
  _toggleList() {
    ...
    this.disclosureList.classList.toggle(classes$5.listVisible);
...
sections.register('footer', {
  onLoad() {
    const disclosureForms = Array.from(
      this.container.querySelectorAll(selectors$4.disclosureForm)
    );
...
class Gallery {
  ...
  setInitialState() {
    const activeImage = this.elements.images.find((image) =>
      image.classList.contains(classes$7.isActive)
    );
  ...
  cacheElement(name) {
    this.elements[name] =
      this.elements[name] ||
      this.elements.container.querySelector(selectors$8[name]);
  }
  ...
  applyActiveClass() {
    this.state.activeMediaImage.classList.add(classes$7.isActive);
  ...
  hideMedia() {
    ...
    this.elements.images.forEach((image) => {
      if (!image.classList.contains(classes$7.isActive)) {
        image
          .closest(selectors$8.mediaWrapper)
          .setAttribute('aria-hidden', 'true');
        return;
      }
      image.closest(selectors$8.mediaWrapper).removeAttribute('aria-hidden');
    });
  }
  ...
  clearActiveClasses() {
    this.elements.images.forEach((image) =>
      image.classList.remove(classes$7.isActive)
    );
  }
  ...
  enableTransition() {
    ...
    this.elements.strip.classList.add(classes$7.transitionReady);
  }
  ...
  resetTransformation() {
    ...
    this.elements.strip.classList.remove(classes$7.transitionReady);
...
class Product {
  ...
  _prepareGallery() {
    this.galleryElement = this.container.querySelector(selectors$6.media);
    ...
    if (this.galleryMediaQueryListener.matches) {
      this._initializeGallery();
      this.gallery.addRoleDescription();
      return;
    }
  ...
  _loadAllGalleryImages() {
    const images = this.container.querySelectorAll(selectors$6.singleMedia);
    ...
    if (this.gallery) {
      this.gallery.elements.images = Array.from(
        this.gallery.elements.container.querySelectorAll(selectors$6.singleMedia)
      );
    }
  }
  ...
  _onFormSubmit(event) {
    event.preventDefault();
    const addToCart = this.container.querySelector(selectors$6.addToCart);
  ...
  _quantityIsInvalid() {
    const quantityInput = this.container.querySelector(selectors$6.quantityInput);
  ...
  _showSuccessMessage() {
    this.successMessage.classList.remove(classes$6.hidden);
  ...
  _hideSuccessMessage() {
    this.successMessage =
      this.successMessage ||
      this.container.querySelector(selectors$6.productSuccessMessage);

    this.successMessage.classList.add(classes$6.hidden);
  }

  _getCartPrice() {
    getState()
      .then((cart) => {
        this._updateCartBubble(cart.total_price);
      })
      .catch((error) => {
        throw new Error(error);
      });
  ...
  _updateCartBubble(price) {
    const cartPriceBubble = document.querySelector(selectors$6.cartPriceBubble);
  ...
  _showErrorMessage(errorMessage) {
    const errorMessageWrapper = this.container.querySelector(
      selectors$6.errorMessageWrapper
    );
    const errorMessageElement = this.container.querySelector(
      selectors$6.errorMessage
    );

    const quantityInputWrapper = this.container.querySelector(
      selectors$6.quantityInputWrapper
    );

    errorMessageElement.innerText = errorMessage;

    if (quantityInputWrapper)
      quantityInputWrapper.classList.add(classes$6.formInputError);

    errorMessageWrapper.classList.remove(classes$6.hidden);
  ...
  _hideErrorMessage() {
    ...
    errorMessageWrapper.classList.add(classes$6.hidden);

    if (quantityInputWrapper)
      quantityInputWrapper.classList.remove(classes$6.formInputError);
  }
  ...
  _updateMasterSelect(variant) {
    ...
    this.masterSelect =
      this.masterSelect ||
      this.container.querySelector(selectors$6.productMasterSelect);
  ...
  _calculateNewInputQuantity(quantitySelector, currentQuantity) {
    const isIncrement = quantitySelector.hasAttribute(
      attributes.dataQuantitySelectorIncrease
    );
  ...
  _updatePrice(variant) {
    this.priceContainer =
      this.priceContainer || this.container.querySelector(selectors$6.price);

    this.priceContainer.classList.remove(
      classes$6.productUnavailable,
      classes$6.productOnSale,
      classes$6.productUnitAvailable,
      classes$6.productSoldOut
    );
    this.priceContainer.removeAttribute('aria-hidden');

    // product unavailable
    if (!variant) {
      this.priceContainer.classList.add(classes$6.productUnavailable);
    ...
    if (!variant.available) {
      this.priceContainer.classList.add(classes$6.productSoldOut);
    }
  ...
  _renderOnSalePrice(variant) {
    const regularPrice = this.priceContainer.querySelectorAll(
      selectors$6.regularPrice
    );
    const salePrice = this.priceContainer.querySelector(selectors$6.salePrice);
    ...
    this.priceContainer.classList.add(classes$6.productOnSale);
  }
  ...
  _updateUnitPrice(variant) {
    ...
    const unitPrice = this.priceContainer.querySelector(selectors$6.unitPrice);
    const unitPriceBaseUnit = this.priceContainer.querySelector(
      selectors$6.unitPriceBaseUnit
    );
    ...
    this.priceContainer.classList.add(classes$6.productUnitAvailable);
  }
  ...
  _updateProductPolicies(variant) {
    const productPolicies = this.container.querySelector(
      selectors$6.productPolicies
    );

    if (!productPolicies) return;

    productPolicies.classList.remove(classes$6.visuallyHidden);

    if (!variant) productPolicies.classList.add(classes$6.visuallyHidden);
  }

  _updateAddToCart(variant) {
    const addToCart = this.container.querySelector(selectors$6.addToCart);
    const addToCartText = this.container.querySelector(selectors$6.addToCartText);
  ...
  _updateLiveRegion(variant) {
    const productStatus = this.container.querySelector(selectors$6.productStatus);
...
class StickyElement {
  ...
  init() {
    ...
    this.stickyElement = this.container.querySelector(selectors$10.stickyElement);
  ...
  _addSentinels() {
    ...
    sentinelTop.classList.add(
      classes$9.stickySentinel,
      classes$9.stickySentinelTop
    );
    sentinelTop.dataset.stickySentinelTop = '';

    this.container.classList.add(classes$9.stickyContainer);
    ...
    sentinelBottom.classList.add(
      classes$9.stickySentinel,
      classes$9.stickySentinelBottom
    );
  ...
  _observeTopSentinel() {
    const topSentinel = this.container.querySelector(
      selectors$10.stickySentinelTop
    );
    ...
    this.topObserver = new IntersectionObserver((records) => {
      records.forEach((record) => {
        ...
        if (startedSticking) {
          this.sticky = true;
          this.stickyElement.classList.add(classes$9.stickyElement);
        }

        if (stoppedSticking) {
          this.sticky = false;
          this.stickyElement.classList.remove(classes$9.stickyElement);
        }
      });
    });
  ...
  _observeBottomSentinel() {
    const bottomSentinel = this.container.querySelector(
      selectors$10.stickySentinelBottom
    );
...
class ViewMore {
  ...
  _loadItems() {
    ...
    fetch(url)
      ...
      .then((html) => {
        const items = [...html.querySelectorAll(selectors$11.viewMoreItem)];
  ...
  _updateButton() {
    ...
    if (this.currentCount >= this.maxCount) {
      this.viewMoreButton.classList.add(classes$10.hidden);
    }
  }

  _updateLiveRegion() {
    const viewMoreStatus = this.container.querySelector(
      selectors$11.viewMoreStatus
    );
...
sections.register('featured-collections', {
  onLoad() {
    this.tabs = this.container.querySelectorAll(
      selectors$9.featuredCollectionsTabs
    );
    ...
    if (this.productModalEnabled) {
      this.productCards = this.container.querySelectorAll(
        selectors$9.productCard
      );
    }

    this.tabsPanel = this.container.querySelectorAll(
      selectors$9.featuredCollectionsTabsPanel
    );
  ...
  _setupProductModal(event) {
    ...
    const collectionTitle = this.container.querySelector(
      selectors$9.featuredCollectionsSelectedTab
  ...
  _getProductModalContent(productUrl) {
    return fetch(productUrl)
      ...
      .then((productHtml) => {
        return productHtml.querySelector(selectors$9.productTemplate);
      })
      .catch((error) => {
        throw new Error(error);
      });
  ...
  _setupProductModalListener() {
    ...
    this.viewCart = this.productModalContent.querySelector(
      selectors$9.productViewCartLink
    );
  ...
  _setupProductModalContent(productTemplate, collectionTitle) {
    this.productModal =
      this.productModal ||
      document.querySelector(selectors$9.productModalWrapper);
    this.productModalContent =
      this.productModalContent ||
      this.productModal.querySelector(selectors$9.productModalContent);

    const productCollectionTitles = productTemplate.querySelectorAll(
      selectors$9.productCollectionTitle
    );

    if (!this._productHasNoMedia(productTemplate))
      this.productModal.classList.add(classes$8.productModalWrapperNoMedia);
    ...
    this.productModalContent
      .querySelectorAll(selectors$9.productImage)
      /* eslint-disable-next-line no-self-assign */
  ...
  _productHasNoMedia(productTemplate) {
    return productTemplate.querySelectorAll(selectors$9.productImage).length;
  },
  ...
  _hideProductModal() {
    ...
    this.productModal.classList.remove(classes$8.productModalWrapperNoMedia);
    this.product.destroy();
  ...
  onViewMoreLoaded(event) {
    ...
    newProducts.forEach((product, index) => {
      ...
      const newProductCard = product.querySelector(selectors$9.productCard);
  ...
  showCollectionPanel(index) {
    ...
    const panelWrapper = this.container.querySelector(
      selectors$9.featuredCollectionsWrapper
    );
    ...
    this.tabs.forEach((tab) => {
      tab.classList.remove(classes$8.featuredCollectionsTabActive);
    ...
    targetTab.classList.add(classes$8.featuredCollectionsTabActive);
    ...
    this.tabsPanel.forEach((tabPanel) => {
      tabPanel.classList.remove(classes$8.featuredCollectionsPanelVisible);
      tabPanel.classList.remove(classes$8.featuredCollectionsPanelActive);
    });

    targetPanel.classList.add(classes$8.featuredCollectionsPanelActive);
    window.requestAnimationFrame(() =>
      targetPanel.classList.add(classes$8.featuredCollectionsPanelVisible)
    );
  },
});
...
(() => {
  const blogTagFilter = document.querySelector(selectors$12.blogTagFilter);
...
(() => {
  ...
  if (passwordInput.hasAttribute(attributes$1.error)) {
...
(() => {
  const container = document.querySelector(selectors$14.customerAddresses);

  if (!container) return;

  const newAddressForm = container.querySelector(selectors$14.addressFormNew);
  ...
  function _setupCountries() {
    ...
    container
      .querySelectorAll(selectors$14.addressCountrySelect)
  ...
  function _toggleAddNewAddressForm(
    ...
    if (newAddressFormExpandedState === 'false') {
      newAddressForm.classList.remove(classes$11.hidden);
      addNewAddressToggle.setAttribute('aria-expanded', true);
    } else {
      newAddressForm.classList.add(classes$11.hidden);
      addNewAddressToggle.setAttribute('aria-expanded', false);
    }

    if (button.hasAttribute(attributes$2.addNewAddressToggle)) return;
  ...
  function _toggleEditAddressForm(event, editAddressToggles) {
    ...
    const editAddressformContainer = container.querySelector(
      selectors$14.editAddressId(addressId)
    );
    ...
    if (editButton.getAttribute('aria-expanded') === 'false') {
      editAddressformContainer.classList.remove(classes$11.hidden);
      editButton.setAttribute('aria-expanded', true);
    } else {
      editAddressformContainer.classList.add(classes$11.hidden);
      editButton.setAttribute('aria-expanded', false);
    }

    if (button.hasAttribute(attributes$2.editAddressFormToggle)) return;
    editButton.focus();
  }
...
(() => {
  const container = document.querySelector(selectors$15.customerLogin);
  ...
  function _setupEventHandlers() {
    const resetPasswordLink = container.querySelector(
      selectors$15.resetPasswordLink
    );

    const cancelResetPasswordLink = container.querySelector(
      selectors$15.cancelResetPasswordLink
    );
    ...
    resetPasswordLink.addEventListener('click', (event) => {
      ...
      const resetPasswordHeading = container.querySelector(
        selectors$15.resetPasswordHeading
      );
    ...
    cancelResetPasswordLink.addEventListener('click', (event) => {
      ...
      const loginHeading = container.querySelector(selectors$15.loginHeading);
  ...
  function _resetPasswordOnSuccess() {
    const resetPasswordStatus = container.querySelector(
      selectors$15.resetPasswordSuccess
    );
    const resetPasswordMessage = container.querySelector(
      selectors$15.resetPasswordSuccessMessage
    );

    if (!resetPasswordStatus) return;

    resetPasswordMessage.classList.remove(classes$12.hidden);
    resetPasswordMessage.focus();
  }

  function _handleContainers(containerHeading, showPasswordPage) {
    const loginContainer = container.querySelector(selectors$15.loginContainer);
    const resetPasswordContainer = container.querySelector(
      selectors$15.resetPasswordContainer
    );

    if (showPasswordPage) {
      loginContainer.classList.add(classes$12.hidden);
      resetPasswordContainer.classList.remove(classes$12.hidden);
    } else {
      loginContainer.classList.remove(classes$12.hidden);
      resetPasswordContainer.classList.add(classes$12.hidden);
    }
